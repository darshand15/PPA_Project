...
void merge(Range in1, Range in2, Range out, Comparator cmp) 
{
    long n1 = in1.size();
    long n2 = in2.size();
    if (n1 + n2 < 10) std::merge(in1.begin(), in1.end(),
                                    in2.begin(), in2.end(),
                                    out.begin(), cmp);
...
{
    long n = in.size();
    if (n < 5) // adjust n to switch to sequential sort for smaller blocks
    {
...
// Enforce constraint and push into tab only if valid
// Setting primary key and foreign key => vector of strings for primary key?
// Delete rows, table => done
// Update fields => done
// Inserting into only specific columns
// Representing NULL values -> bitmap for the fields within each row?
// Select conditions with and, or
