...
int main()
{
    ...
    SELECT_PK_EQ(Person, id, ==, 17)

    std::cout << "Select PK Range \n";
    SELECT_PK_RANGE(Person, id, 13, 47)

    std::cout << "Select Cols Cond \n";
    SELECT_COLS_COND_PAR(Person,
        ((id))
        ((fname))
        ((age)),
        age, >=, 25
    ...
    SELECT_ALL_COND_PAR(Person, salary, >=, 25)

    std::cout << "Order by Asc \n";
    ORDER_BY_ASC_PAR(Person,
        ((fname))
        ((id))
    )

    std::cout << "Order by Desc \n";
    ORDER_BY_DESC_PAR(Person,
        ((salary))
        ((id))
    )

    std::cout << "Group By Count \n";
    GROUP_BY_COUNT_PAR2(Person,
        ((lname))
    )

    std::cout << "Group By Min \n";
    GROUP_BY_MIN_PAR2(Person,
        ((country)), salary
    )

    std::cout << "Group By Max \n";
    GROUP_BY_MAX_PAR2(Person,
        ((country)), age
    )

    std::cout << "Group By Sum \n";
    GROUP_BY_SUM_PAR2(Person,
        ((country)), salary
    )

    std::cout << "Group By Avg \n";
    GROUP_BY_AVG_PAR2(Person,
        ((country)), salary
    )
